...
def _make_adaln_forward(base, a, b, shared):
    ...
    def forward(t_emb):
        ...
        if st is not None:
            ...
            low_rank = F.linear(sv, av)
            torch.addmm(x, low_rank, bv.T, beta=1.0, out=x)
...
class _FrugalLoRA(comfy.weight_adapter.LoRAAdapter):
    ...
    transiently. optimized runtime sends the second low-rank GEMM through addmm with base_out
    as C/output, so the LoRA update is accumulated by the GEMM epilogue without
    materializing a full-size update tensor or launching a separate add kernel.
    base_out is the module's fresh output, so the in-place epilogue is safe.
    Linear-only (all H3 lora modules are Linear); anything else falls back to the
    stock path."""
    ...
    def bypass_forward(self, org_forward, x, *args, **kwargs):
        ...
        base_2d = base_out.reshape(-1, base_out.shape[-1])
        low_rank_2d = low_rank.reshape(-1, low_rank.shape[-1])
        torch.addmm(base_2d, low_rank_2d, up.T, beta=1.0, alpha=scale, out=base_2d)
        return base_out
