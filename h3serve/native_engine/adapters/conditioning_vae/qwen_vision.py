"""Native Qwen3-VL vision tower for the packed local H3 conditioner.

Derived from the Apache-2.0 LightX2V implementation at revision
``231e2307f15b9eb60fe3f877f7eed945c8d8d717``.  The loader is rewritten for
the release's single-file ``visual.*`` checkpoint instead of LightX2V's
official sharded ``model.visual.*`` layout.  No LightX2V package import or
...
class MiniMaxH3Qwen3VLVisionTower(nn.Module):
    ...
    @classmethod
    def from_single_file(cls, checkpoint_path, vision_config, *, prefix="visual."):
        """Load all persistent vision tensors from one local safetensors file."""
        ...
        if not state:
            raise KeyError(f"checkpoint contains no {prefix} tensors")
