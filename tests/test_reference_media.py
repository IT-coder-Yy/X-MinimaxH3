...
class Ref2VAContractTest(unittest.TestCase):
    def test_release_tokenizer_directory_is_processor_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            tokenizer = root / "tokenizer"
            tokenizer.mkdir()
            checkpoint = root / "qwen.safetensors"
            checkpoint.write_bytes(b"contract-only")
            conditioner = PackedQwen3VLT2AVConditioner(
                checkpoint,
                tokenizer,
                device="cuda",
            )
            self.assertEqual(conditioner.processor_path, tokenizer)
            self.assertEqual(
                conditioner.model_config_path,
    ...
    def test_reference_video_decode_resample_and_temporal_alignment(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(prepared[0].frames.shape, (39, 48, 64, 3))
